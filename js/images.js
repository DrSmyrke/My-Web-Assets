var images = {
	...
	'zoom-in': { 'width': 24, 'height': 24, 'img': '<circle class="stroke" cx="11" cy="11" r="8"></circle><line class="stroke" x1="21" y1="21" x2="16.65" y2="16.65"></line><line class="stroke" x1="11" y1="8" x2="11" y2="14"></line><line class="stroke" x1="8" y1="11" x2="14" y2="11"></line>' },
	'zoom-out': { 'width': 24, 'height': 24, 'img': '<circle class="stroke" cx="11" cy="11" r="8"></circle><line class="stroke" x1="21" y1="21" x2="16.65" y2="16.65"></line><line class="stroke" x1="8" y1="11" x2="14" y2="11"></line>' },
	'plus-square': { 'width': 24, 'height': 24, 'img': '<rect class="stroke" x="3" y="3" width="18" height="18" rx="2" ry="2"></rect><line class="stroke" x1="12" y1="8" x2="12" y2="16"></line><line class="stroke" x1="8" y1="12" x2="16" y2="12"></line>' },
	...
	'rotate-ccw': { 'width': 24, 'height': 24, 'img': '<polyline class="stroke" points="1 4 1 10 7 10"></polyline><path class="stroke" d="M3.51 15a9 9 0 1 0 2.13-9.36L1 10"></path>' },
	...
	'upload-cloud': { 'width': 24, 'height': 24, 'img': '<polyline class="stroke" points="16 16 12 12 8 16"></polyline><line class="stroke" x1="12" y1="12" x2="12" y2="21"></line><path class="stroke" d="M20.39 18.39A5 5 0 0 0 18 9h-1.26A8 8 0 1 0 3 16.3"></path><polyline class="stroke" points="16 16 12 12 8 16"></polyline>' },
	'download-cloud': { 'width': 24, 'height': 24, 'img': '<polyline class="stroke" points="8 17 12 21 16 17"></polyline><line class="stroke" x1="12" y1="12" x2="12" y2="21"></line><path class="stroke" d="M20.88 18.09A5 5 0 0 0 18 9h-1.26A8 8 0 1 0 3 16.29"></path>' },
...
window.addEventListener( 'load', function( ev ){
	...
	for( let i = 0; i < list.length; i++ ){
		...
		if( images[ img.title ] == undefined ) continue;
		
		let svg = getSvgFromImageElement( img.title, img );
		...
		if( app.debug && app.debugAll ) console.info( img, svg );
	}
}, false);

// -------------------------------------------------------------
function getSvgFromImageName( name = '', backquotes = false )
{
	if( app.debug ) console.log( 'getSvgFromImageName >:', name );
	
	let svg = undefined;

	if( images[ name ] != undefined ){
		let data			= images[ name ];
		svg					= getSVG( '', data.width, data.height, data.img,  '', undefined );
	}
...
// -------------------------------------------------------------
function getSvgFromImageElement( name = '', imageElement = undefined )
{
	if( app.debug ) console.log( 'getSvgFromImageElement >:', name, imageElement );
	
	let svg = undefined;

	if( images[ name ] != undefined && imageElement != undefined ){
		let data			= images[ name ];
		svg					= getSVG( imageElement.className, data.width, data.height, data.img, imageElement.id, imageElement.onclick, imageElement.onmouseover );
	}
...
// -------------------------------------------------------------
function getSVG( className = '', width = 0, height = 0, innerHTML = '', id = '', onclick = undefined, onmouseover = undefined )
{
	if( app.debug && app.debugAll ) console.log( 'getSVG >:', className, width, height, innerHTML, id, onclick );
